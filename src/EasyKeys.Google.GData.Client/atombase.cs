...
namespace EasyKeys.Google.GData.Client
{
    ...
    public abstract class AtomBase : IExtensionContainer, IVersionAware
    {
        ...
        {
            WalkTree(new BaseMarkDirty(fFlag));
        }
        ...
        {
            get { return _uriImpliedBase; }
            set { _uriImpliedBase = value; }
        }
        ...
        {
            return Utilities.CalculateUri(Base, ImpliedBase, part);
        }
        ...
        {
            get { return _atomLanguageTag; }
            set { Dirty = true; _atomLanguageTag = value; }
        }
        ...
        public ExtensionList ExtensionFactories
        {
            get
            {
                if (_extensionFactories == null)
                {
                    _extensionFactories = new ExtensionList(this);
                }
        ...
        public ExtensionList ExtensionElements
        {
            get
            {
                if (_extensionsList == null)
                {
                    _extensionsList = new ExtensionList(this);
                }

                return _extensionsList;
            }
        }
        ...
        {
            return Utilities.FindExtensions(
                ExtensionElements,
                localName, ns, arr);
        }
        ...
        {
            return Utilities.FindExtensions<T>(
                ExtensionElements,
                localName, ns);
        }
        ...
        protected virtual void SaveXmlAttributes(XmlWriter writer)
        {
            ...
            if (Utilities.IsPersistable(_uriBase))
            {
                // Lookup the prefix and then write the ISBN attribute.
                writer.WriteAttributeString("xml", "base", BaseNameTable.NSXml, Base.ToString());
            }

            if (Utilities.IsPersistable(_atomLanguageTag))
            {
                writer.WriteAttributeString("xml", "lang", BaseNameTable.NSXml, Language);
            }
            ...
            foreach (IExtensionElementFactory ele in ExtensionElements)
            {
        ...
        {
            if (Utilities.IsPersistable(_uriBase) || Utilities.IsPersistable(_atomLanguageTag))
            {
                return true;
            }

            if (_extensionsList != null && _extensionsList.Count > 0 && _extensionsList[0] != null)
            {
                return true;
            }
