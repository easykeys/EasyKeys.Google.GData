...
namespace EasyKeys.Google.GData.Client
{
    ...
    public class AtomFeedParser : BaseFeedParser
    {
        ...
        private AtomParserNameTable _nameTable;
        ...
        public override void Parse(Stream streamInput, AtomFeed feed)
        {
            ...
            try
            {
                XmlReader reader = new XmlTextReader(streamInput, _nameTable.Nametable);
        ...
        protected void ParseFeed(XmlReader reader, AtomFeed feed)
        {
            ...
            object localname = reader.LocalName;
            Tracing.TraceInfo("localname is: " + reader.LocalName);
            if (localname.Equals(_nameTable.Feed))
            {
            ...
            else if (localname.Equals(_nameTable.Entry))
            {
        ...
        protected bool ParseBaseAttributes(XmlReader reader, AtomBase baseObject)
        {
            ...
            {
                if (localName.Equals(_nameTable.Base))
                {
                    baseObject.Base = new AtomUri(reader.Value);
                    fRet = false;
                }
                else if (localName.Equals(_nameTable.Language))
                {
            ...
            else if (IsCurrentNameSpace(reader, BaseNameTable.gNamespace))
            {
                ...
                if (se != null)
                {
                    if (localName.Equals(_nameTable.ETag))
                    {
        ...
        protected AtomPerson ParsePerson(XmlReader reader, AtomBase owner)
        {
            ...
            while (NextChildElement(reader, ref lvl))
            {
                ...
                if (localname.Equals(_nameTable.Name))
                {
                ...
                else if (localname.Equals(_nameTable.Uri))
                {
                    author.Uri = new AtomUri(Utilities.DecodedValue(reader.ReadString()));
                    reader.Read();
                }
                else if (localname.Equals(_nameTable.Email))
                {
        ...
        protected AtomCategory ParseCategory(XmlReader reader, AtomBase owner)
        {
            ...
            if (category != null)
            {
                ...
                if (reader.HasAttributes)
                {
                    ...
                    {
                        object localname = reader.LocalName;
                        if (localname.Equals(_nameTable.Term))
                        {
                            category.Term = Utilities.DecodedValue(reader.Value);
                        }
                        else if (localname.Equals(_nameTable.Scheme))
                        {
                            category.Scheme = new AtomUri(reader.Value);
                        }
                        else if (localname.Equals(_nameTable.Label))
                        {
        ...
        protected AtomLink ParseLink(XmlReader reader, AtomBase owner)
        {
            ...
            if (reader.HasAttributes)
            {
                ...
                {
                    localname = reader.LocalName;
                    if (localname.Equals(_nameTable.HRef))
                    {
                        link.HRef = new AtomUri(reader.Value);
                    }
                    else if (localname.Equals(_nameTable.Rel))
                    {
                        link.Rel = Utilities.DecodedValue(reader.Value);
                    }
                    else if (localname.Equals(_nameTable.Type))
                    {
                        link.Type = Utilities.DecodedValue(reader.Value);
                    }
                    else if (localname.Equals(_nameTable.HRefLang))
                    {
                    ...
                    else if (localname.Equals(_nameTable.Length))
                    {
        ...
        protected void ParseBatch(XmlReader reader, AtomEntry entry)
        {
            ...
            if (IsCurrentNameSpace(reader, BaseNameTable.gBatchNamespace))
            {
                ...
                if (elementName.Equals(_nameTable.BatchId))
                {
                    batch.Id = Utilities.DecodedValue(reader.ReadString());
                }
                else if (elementName.Equals(_nameTable.BatchOperation))
                {
                    batch.Type = ParseOperationType(reader);
                }
                else if (elementName.Equals(_nameTable.BatchStatus))
                {
                    batch.Status = GDataBatchStatus.ParseBatchStatus(reader, this);
                }
                else if (elementName.Equals(_nameTable.BatchInterrupt))
                {
            ...
            if (reader.NodeType == XmlNodeType.Element)
            {
                if (reader.HasAttributes)
                {
                    while (reader.MoveToNextAttribute())
                    {
                        ...
                        if (attributeName.Equals(_nameTable.Type))
                        {
            ...
            if (generator != null)
            {
                ...
                if (reader.HasAttributes)
                {
                    while (reader.MoveToNextAttribute())
                    {
                        ...
                        if (attributeName.Equals(_nameTable.Uri))
                        {
                            generator.Uri = new AtomUri(reader.Value);
                        }
                        else if (attributeName.Equals(_nameTable.Version))
                        {
            ...
            if (content != null)
            {
                if (reader.HasAttributes)
                {
                    while (reader.MoveToNextAttribute())
                    {
                        ...
                        else if (localname.Equals(_nameTable.Src))
                        {
