...
namespace EasyKeys.Google.GData.Client
{
    ...
    public class Feed<T> where T : Entry, new()
    {
        ...
        public AtomFeed AtomFeed
        {
            get
            {
                if (_af == null)
                {
                    if (_service != null && _query != null)
                    {
                        _af = _service.Query(_query);
                    }
                }
        ...
        public IEnumerable<T> Entries
        {
            get
            {
                ...
                AtomFeed originalFeed = AtomFeed;

                _numberRetrieved = 0;

                do
                {
                    looping = _af.NextChunk != null && _paging;
                    foreach (AtomEntry e in _af.Entries)
                    {
                        T t = new T();
                        if (t != null)
                        {
                        ...
                        if (Maximum > 0 && _numberRetrieved >= Maximum)
                        {
                            yield break;
                        }
                    }
                    ...
                    if (looping)
                    {
                        FeedQuery q = new FeedQuery(AtomFeed.NextChunk);
                        FeedQuery.PrepareQuery(q, _settings);
                        _af = AtomFeed.Service.Query(q);
                    }
                } while (looping);
    ...
    public abstract class Entry
    {
        ...
        public string Id
        {
            ...
            {
                EnsureInnerObject();
                return _e.Id.AbsoluteUri;
            }

            set
            {
                EnsureInnerObject();
                _e.Id = new AtomId(value);
            }
        }
        ...
        public string Author
        {
            ...
            {
                EnsureInnerObject();
                if (_e.Authors.Count > 0 && _e.Authors[0] != null)
                {
                    return _e.Authors[0].Name;
                }
        ...
        public string Content
        {
            ...
            {
                EnsureInnerObject();
                return _e.Content.Content;
            }

            set
            {
                EnsureInnerObject();
                _e.Content.Content = value;
            }
        }
        ...
        public string Summary
        {
            ...
            {
                EnsureInnerObject();
                return _e.Summary.Text;
            }

            set
            {
                EnsureInnerObject();
                _e.Summary.Text = value;
            }
        }
        ...
        public MediaSource MediaSource
        {
            ...
            {
                EnsureInnerObject();
                AbstractEntry ae = _e as AbstractEntry;
    ...
    public class RequestSettings
    {
        ...
        private Uri _clientLoginHandler;
        ...
        /// <returns></returns>
        public RequestSettings(string applicationName, string username, string password) : this(applicationName, new GDataCredentials(username, password))
        {
        }
        ...
        {
            _authType = AuthenticationType.clientLogin;
            _applicationName = applicationName;
        ...
        public RequestSettings(string applicationName, string authSubToken)
            : this(applicationName)
        {
            _authType = AuthenticationType.authSub;
        ...
        public RequestSettings(
            ...
            : this(applicationName)
        {
            _authType = AuthenticationType.oAuth2;
            OAuth2Parameters = parameters;
        }
        ...
        public Uri ClientLoginHandler
        {
            get
            {
                return _clientLoginHandler != null ?
                    _clientLoginHandler : new Uri(GoogleAuthentication.UriHandler);
            }

            set { _clientLoginHandler = value; }
        }
        ...
        public HttpWebRequest CreateHttpWebRequest(string serviceName, string httpMethod, Uri targetUri)
        {
            ...
            if (_authType == AuthenticationType.clientLogin)
            {
                EnsureClientLoginCredentials(request, serviceName);
            }

            if (_authType == AuthenticationType.authSub)
            {
    ...
    public abstract class FeedRequest<T> where T : Service
    {
        ...
        protected void PrepareService(Service s)
        {
            ...
            {
                s.Credentials = _settings.Credentials;
            }

            if (_settings.AuthSubToken != null)
            {
                GAuthSubRequestFactory authFactory = new GAuthSubRequestFactory(s.ServiceIdentifier, _settings.Application);
                authFactory.UserAgent = authFactory.UserAgent + "--IEnumerable";
                authFactory.Token = _settings.AuthSubToken;
                authFactory.PrivateKey = _settings.PrivateKey;
                s.RequestFactory = authFactory;
            }
            else if (_settings.ConsumerKey != null)
            {
                // let's create an OAuth factory
                GOAuthRequestFactory authFactory = new GOAuthRequestFactory(s.ServiceIdentifier, _settings.Application);
                authFactory.ConsumerKey = _settings.ConsumerKey;
                authFactory.ConsumerSecret = _settings.ConsumerSecret;
                authFactory.Token = _settings.Token;
                authFactory.TokenSecret = _settings.TokenSecret;
                s.RequestFactory = authFactory;
            }
            else if (_settings.OAuth2Parameters != null)
            {
                s.RequestFactory = new GOAuth2RequestFactory(s.ServiceIdentifier, _settings.Application, _settings.OAuth2Parameters);
            }
            else
            {
            ...
            if (_settings.Timeout != -1)
            {
                GDataRequestFactory f = s.RequestFactory as GDataRequestFactory;
                if (f != null)
                {
                    f.Timeout = _settings.Timeout;
                }
            }

            s.RequestFactory.UseSSL = _settings.UseSSL;
        }
        ...
        protected Uri CreateUri(string location)
        {
            ...
            if (_settings.OAuthUser != null && location.IndexOf(OAuthUri.OAuthParameter) != 0)
            {
                retUri = new OAuthUri(location, _settings.OAuthUser, _settings.OAuthDomain);
            }
            else
            {
        ...
        protected virtual Feed<Y> PrepareFeed<Y>(FeedQuery q) where Y : Entry, new()
        {
            ...
            f.AutoPaging = _settings.AutoPaging;
            f.Maximum = _settings.Maximum;
            return f;
        }
        ...
        {
            return new Feed<Y>(_atomService, q);
        }
        ...
        public IWebProxy Proxy
        {
            get
            {
                GDataRequestFactory x = _atomService.RequestFactory as GDataRequestFactory;
                if (x != null)
                {
                    return x.Proxy;
                }
        ...
        public Feed<Y> Parse<Y>(Stream inputStream, Uri targetUri) where Y : Entry, new()
        {
            ...
            AtomFeed feed = Service.CreateAndParseFeed(inputStream, targetUri);
        ...
        public Feed<Y> Batch<Y>(List<Y> entries, Uri batchUri, GDataBatchOperationType defaultOperation) where Y : Entry, new()
        {
            if (entries.Count > 0)
            {
                ...
                AtomFeed resultFeed = Service.Batch(batchFeed, q.Uri);
        ...
        public Y Update<Y>(Y entry) where Y : Entry, new()
        {
            ...
            Stream s = Service.EntrySend(q.Uri, entry.AtomEntry, GDataRequestType.Update, null);
            AtomEntry ae = Service.CreateAndParseEntry(s, new Uri(entry.AtomEntry.EditUri.ToString()));
        ...
        public Y Insert<Y>(Uri address, Y entry) where Y : Entry, new()
        {
            ...
            AtomEntry ae = Service.Insert(q.Uri, entry.AtomEntry);
            if (ae != null)
            {
                r = new Y();
                r.AtomEntry = ae;
