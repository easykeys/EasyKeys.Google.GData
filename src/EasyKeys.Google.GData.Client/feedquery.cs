...
namespace EasyKeys.Google.GData.Client
{
    ...
    public class QueryCategory
    {
        ...
        private AtomCategory _category;

        /// <summary>Boolean operator (can be OR or AND).</summary> 
        private QueryCategoryOperator _categoryOperator;
    ...
    public class FeedQuery : ISupportsEtag
    {
        ...
        private QueryCategoryCollection _categories;
        ...
        public Uri Uri
        {
            get
            {
                ...
                String uriToUse = computedBaseUri == null ? String.Empty : computedBaseUri.Replace(UnusedProtocol, DefaultProtocol);
        ...
        {
            get { return _useCategoryQueriesAsParameter; }
            set { _useCategoryQueriesAsParameter = value; }
        }
        ...
        public bool UseSSL
        {
            get { return _defaultSSL; }
            set { _defaultSSL = value; }
        }
        ...
        {
            get { return _oauthRequestorId; }
            set { _oauthRequestorId = value; }
        }
        ...
        public QueryCategoryCollection Categories
        {
            get
            {
                if (_categories == null)
                {
                    _categories = new QueryCategoryCollection();
                }
        ...
        {
            get { return _publishedMin; }
            set { _publishedMin = value; }
        }
        ...
        {
            get { return _publishedMax; }
            set { _publishedMax = value; }
        }
        ...
        {
            get { return _numToRetrieve; }
            set { _numToRetrieve = value; }
        }
        ...
        {
            get { return _altFormat; }
            set { _altFormat = value; }
        }
        ...
        protected virtual Uri ParseUri(Uri targetUri)
        {
                    ...
                    if (token.Length > 0)
                    {
                        ...
                        switch (parameters[0])
                        {
                            ...
                            case "author":
                                ...
                                break;
                            case "start-index":
                                StartIndex = int.Parse(parameters[1], CultureInfo.InvariantCulture);
                                break;
                            case "max-results":
                                NumberToRetrieve = int.Parse(parameters[1], CultureInfo.InvariantCulture);
                                break;
                            case "updated-min":
                                StartDate = DateTime.Parse(parameters[1], CultureInfo.InvariantCulture);
                                break;
                            case "updated-max":
                                EndDate = DateTime.Parse(parameters[1], CultureInfo.InvariantCulture);
                                break;
                            case "published-min":
                                MinPublication = DateTime.Parse(parameters[1], CultureInfo.InvariantCulture);
                                break;
                            case "published-max":
                                MaxPublication = DateTime.Parse(parameters[1], CultureInfo.InvariantCulture);
                                break;
                            case "category":
                                ParseCategoryQueryString(parameters[1]);
                                break;
                            case "xoauth_requestor_id":
                                OAuthRequestorId = parameters[1];
                                break;
                            default:
                                break;
                        }
            ...
            if (newPath != null)
            {
                ...
                baseUri = newUri.Uri.AbsoluteUri;
                UseSSL = baseUri.StartsWith("https://");
            }
        ...
        internal static string CleanPart(string part)
        {
        ...
        protected virtual string CalculateQuery(string basePath)
        {
            ...
            if (FeedFormat != AlternativeFormat.Atom)
            {
                newPath.Append(paramInsertion);
                newPath.AppendFormat(CultureInfo.InvariantCulture, "alt={0}", FormatToString(FeedFormat));
                paramInsertion = '&';
            }

            paramInsertion = AppendQueryPart(Query, "q", paramInsertion, newPath);
            paramInsertion = AppendQueryPart(Author, "author", paramInsertion, newPath);
            paramInsertion = AppendQueryPart(StartDate, "updated-min", paramInsertion, newPath);
            paramInsertion = AppendQueryPart(EndDate, "updated-max", paramInsertion, newPath);
            paramInsertion = AppendQueryPart(MinPublication, "published-min", paramInsertion, newPath);
            paramInsertion = AppendQueryPart(MaxPublication, "published-max", paramInsertion, newPath);
            paramInsertion = AppendQueryPart(StartIndex, 0, "start-index", paramInsertion, newPath);
            paramInsertion = AppendQueryPart(NumberToRetrieve, 0, "max-results", paramInsertion, newPath);
            paramInsertion = AppendQueryPart(OAuthRequestorId, "xoauth_requestor_id", paramInsertion, newPath);
        ...
        private char CreateCategoryString(StringBuilder builder, char connect)
        {
            ...
            string prePendString = CategoryQueriesAsParameter ? connect + "category=" : "/-/";
            string seperator = CategoryQueriesAsParameter ? "," : "/";
            ...
            if (builder.Length > iLen && CategoryQueriesAsParameter)
            {
                return '&';
            }
