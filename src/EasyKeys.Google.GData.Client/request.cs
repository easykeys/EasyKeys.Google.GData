...
namespace EasyKeys.Google.GData.Client
{
    ...
    public class GDataCredentials
    {
        ...
        private string _accountType = GoogleAuthentication.AccountTypeDefault;
    ...
    public class GDataRequestFactory : IGDataRequestFactory
    {
        ...
        private List<string> _customHeaders;
        ...
        {
            _userAgent = Utilities.ConstructUserAgent(userAgent, GetType().Name);
            _keepAlive = true;
        }
        ...
        public string Slug
        {
            get { return _slugHeader; }

            set
            {
                _slugHeader = Utilities.EncodeSlugHeader(value);
            }
        }
        ...
        public List<string> CustomHeaders
        {
            get
            {
                ...
                {
                    _customHeaders = new List<string>();
                }
    ...
    public class GDataRequest : IGDataRequest, IDisposable, ISupportsEtag
    {
        ...
        private GDataRequestType _type;
        ...
        private GDataRequestFactory _factory; // holds the factory to use
        ...
        private AtomBase _contentStore;
        ...
        public string ContentType
        {
            get { return _contentType == null ? _factory.ContentType : _contentType; }
            set { _contentType = value; }
        }
        ...
        public string Slug
        {
            get { return _slugHeader == null ? _factory.Slug : _slugHeader; }
        ...
        {
            if (_webRequest == null && _targetUri != null)
            {
                if (_factory.UseSSL && !_targetUri.Scheme.ToLower().Equals("https"))
                {
                    _targetUri = new Uri("https://" + _targetUri.Host + _targetUri.PathAndQuery);
                }

                _webRequest = WebRequest.Create(_targetUri);
                ...
                HttpWebRequest web = _webRequest as HttpWebRequest;
                ...
                if (web != null)
                {
                    ...
                    web.UserAgent = _factory.UserAgent;
                    web.KeepAlive = _factory.KeepAlive;
                    web.CookieContainer = _factory.Cookies;

                    // add all custom headers
                    if (_factory.hasCustomHeaders)
                    {
                        foreach (string s in _factory.CustomHeaders)
                        {
                            Request.Headers.Add(s);
                        }
                    }

                    if (_factory.Timeout != -1)
                    {
                        web.Timeout = _factory.Timeout;
                    }
                    ...
                    if (_factory.Proxy != null)
                    {
                        Request.Proxy = _factory.Proxy;
                    }
                }
        ...
        protected virtual void EnsureCredentials()
        {
            if (Credentials != null)
            {
                _webRequest.Credentials = Credentials.NetworkCredential;
            }
        }
        ...
        public virtual void Execute()
        {
            try
            {
                ...
                LogRequest(_webRequest);
                _webResponse = _webRequest.GetResponse();
            }
            catch (WebException e)
            {
                Tracing.TraceCall("GDataRequest::Execute failed: " + _targetUri.ToString());
                GDataRequestException gde = new GDataRequestException("Execution of request failed: " + _targetUri.ToString(), e);
                throw gde;
            }
            ...
            LogResponse(_webResponse);
            if (_webResponse is HttpWebResponse)
            {
                HttpWebResponse response = _webResponse as HttpWebResponse;
                HttpWebRequest request = _webRequest as HttpWebRequest;

                _useGZip = (string.Compare(response.ContentEncoding, "gzip", true, CultureInfo.InvariantCulture) == 0);
                if (_useGZip)
                {
                ...
                if (response.StatusCode == HttpStatusCode.Forbidden)
                {
                    ...
                    throw new GDataForbiddenException(
                        "Execution of request returned HttpStatusCode.Forbidden: " +
                        _targetUri.ToString() + response.StatusCode.ToString(), _webResponse);
                }

                if (response.StatusCode == HttpStatusCode.Conflict)
                {
                    // a put went bad due to a version conflict
                    throw new GDataVersionConflictException(
                        "Execution of request returned HttpStatusCode.Conflict: " +
                        _targetUri.ToString() + response.StatusCode.ToString(), _webResponse);
                }
                    ...
                    && response.StatusCode == HttpStatusCode.NotModified)
                {
                    ...
                    throw new GDataNotModifiedException("Content not modified: " + _targetUri.ToString(), _webResponse);
                }
                    ...
                    response.StatusCode == HttpStatusCode.RedirectKeepVerb)
                {
                    ...
                    throw new GDataRedirectException("Execution resulted in a redirect from " + _targetUri.ToString(), _webResponse);
                }
                ...
                if (code > 299)
                {
                    ...
                    throw new GDataRequestException(
                        "Execution of request returned unexpected result: " + _targetUri.ToString() +
                        response.StatusCode.ToString(), _webResponse);
                }

                _contentLength = response.ContentLength;

                // if we got an etag back, remember it
                _eTag = response.Headers[GDataRequestFactory.EtagHeader];
