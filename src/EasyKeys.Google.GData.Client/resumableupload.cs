...
/// </summary>
namespace EasyKeys.Google.GData.Client.ResumableUpload
{
    internal class AsyncResumableUploadData : AsyncData, IAsyncEntryData
    {
        private Authenticator _authenticator;
        private string _contentType;
        private AbstractEntry _entry;
    ...
    public class ResumableUploader : AsyncDataHandler
    {
        ...
        private Dictionary<Uri, long> _lastChunks = new Dictionary<Uri, long>(); // keeps track of the indexes of the last chunk for each upload
            ...
            Stream payload, string mediaType, AsyncData data)
        {
            ...
            int index = 0;
            if (!_lastChunks.ContainsKey(sessionUri))
            {
                _lastChunks.Add(sessionUri, 0);
            }
            ...
            try
            {
                ...
                {
                    index = (int)((double)payload.Position / (_chunkSize * ResumableUploader.MB));
                }
            }
        ...
        {
            long chunkCounter = 0;
            long chunkStart = _lastChunks[requestId];
            long chunkSizeMb = _chunkSize * ResumableUploader.MB;
            long dataLength;
            ...
            _lastChunks[requestId] = chunkEnd; // save the last start index, need to add 503 error handling to this
